...
searchPlayerHeader1 = {
    'authorization' : os.getenv('SEARCH_PLAYER_HEADER_1')
} 

while 1:
        
        #on récup les 20 premiers clans par ordre alphabétique Z-A qui n'ont pas encore ete check dans la db 
        cur.execute('SELECT id FROM clanFR WHERE lastChecked IS NULL ORDER BY id ASC LIMIT 20 OFFSET 4000')
        ...
        if clanResult == []:
            ...
            for clan in clanResult: # pour chaque clan
                try:
                    ...
                    for member in clan['memberList']: # pour chaque membre du clan
                        try:
                            ...
                            if resultat == None: # si il n'y a pas deja le ce clan dans la base de donnée 
                                ...
                                print(datetime.now(),"->",f"player {member['tag']} inserted !", end='\r')
                                ...
                                print(datetime.now(),"->",f"player {member['tag']} updated !", end='\r')
                    ...
                    conn.commit() # on enregistre les modification de la data base 
                    print(datetime.now(),"-> ",f"clan {clan['tag']} checked !", end='\r')
